...
pub fn run(a: Args) -> utz_build::Result<()> {
    ...
    let rings: Vec<(u16, Vec<(f64, f64)>)> = feats
        .iter()
        .enumerate()
        .flat_map(|(fid, f)| {
            f.polys
                .iter()
                .flatten()
                .map(move |r| (u16::try_from(fid).expect("feature id fits u16"), r.clone()))
        })
        .collect();
    ...
    let vec_hdr32 = 12usize;
    let vec_hdr64 = 24usize;

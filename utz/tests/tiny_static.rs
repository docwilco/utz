...
fn garbage_bytes_report_precise_errors() {
    ...
    let truncated = utz::Finder::from_static(b"junk")
        .err()
        .expect("too short to parse");
    assert_eq!(truncated, utz::Error::Truncated);
    let bad_magic = utz::Finder::from_static(&[0u8; 16])
        .err()
        .expect("wrong magic");
...
fn new_borrows_the_tiny_static_preset() {
    let finder = utz::Finder::new().expect("tiny-static asset parses");
    assert!(

...
pub fn run(_a: Args) -> utz_build::Result<()> {
    ...
    while n < 8000 {
        let lo = next() * 360.0 - 180.0;
        let la = next() * 180.0 - 90.0;
        n += 1;
        let t = look(&f64p, Point::new(lo, la));
        if look(&i64p, Point::new(q64(lo), q64(la))) != t {
            d64 += 1;
        }
        if look(&i32p, Point::new(q32(lo), q32(la))) != t {
            d32 += 1;
        }
    }
    ...
    #[expect(
        ...
        reason = "d64/d32 ≤ n = 8000 sample points; percentage display"
    )]
...
fn poly_f64(ext: &[(f64, f64)], holes: &[Vec<(f64, f64)>]) -> Polygon<f64> {
    Polygon::new(
        ...
        holes.iter().map(|h| LineString::from(h.clone())).collect(),
    )
}
fn poly_i<T: geo_types::CoordNum>(
    ...
    let cv = |v: &[(f64, f64)]| -> LineString<T> {
        LineString::from(
            v.iter()
                .map(|&(x, y)| (f((x * s).round()), f((y * s).round())))
                .collect::<Vec<_>>(),
        )
    };

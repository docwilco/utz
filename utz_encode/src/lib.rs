...
//! container. It covers shared-arc topology + simplification (topo),
//! quantization, grid prefilter (grid), and the delta+varint container
//! serializer with its generic-compression codecs (encode).
//!
//! Split out of `utz_build` so it compiles for wasm32-unknown-unknown: the
//! webdist viewer runs this exact pipeline live for size stats (the wasm
//! surface lives in `utz_viz`).
